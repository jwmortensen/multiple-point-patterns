...
setwd("~/statistics/projects/houston_heat/reviewer_response/final")
load("./RData/Spatial911PtPtrn.RData")
load("./RData/AllTempData.RData")
load("./RData/intercept_data_30_nov_2016.RData")
sourceCpp("./estimateParametersLowerMu.cpp")
death <- read.csv("./RData/cleanDeathData.csv", header=T)
intercept.df$Population <- intercept.df$TotalPop
...
MHGibbs <- function(ndraws, thin.factor, init.lstar.911, init.lstar.death, init.lstar.mu, model.index) {
  # Adaptive MCMC stuff
  ...
  intercept <- model.matrix(model_vars[[model.index]], data = intercept.df)

  pb <- txtProgressBar(min=0, max=ndraws*thin.factor, style=3)
  n <- ifelse(thin.factor == 1, 1, 0)

  # initialize containers to hold drawsta
  lstar.mu <- lstar.911 <- lstar.death <- lstar.mu <- matrix(NA, nrow=ndraws, ncol=num.pred.locs)
  ...
  beta[1, ] <- temp.beta <- rep(0, ncol(intercept))
  d.vals[1] <- 0
...
model_vars <- list(fit_himax = as.formula("~1 + HI_MAX"),
                   fit_himin = as.formula("~1 + HI_MIN"),
                   fit_t2max = as.formula("~1 + T2MAX"),
                   fit_t2min = as.formula("~1 + T2MIN"),
                   fit_swmin = as.formula("~1 + SW_MIN"),
                   fit_swmax = as.formula("~1 + SW_MAX"),
                   fit_02 = as.formula("~1 + HI_MAX + over65PCT + under5PCT"),
                   fit_03 = as.formula("~1 + HI_MAX + blackPCT + hispanicPCT"),
                   fit_04 = as.formula("~1 + HI_MAX + NOACPCT + SQLPCT
                                       + med_mkt_val"),
                   fit_05 = as.formula("~1 + HI_MAX + over65PCT + under5PCT +
                                       blackPCT + hispanicPCT"),
                   fit_06 = as.formula("~1 + HI_MAX + over65PCT + under5PCT +
                                       NOACPCT + SQLPCT + med_mkt_val"),
                   fit_07 = as.formula("~1 + blackPCT + hispanicPCT +
                                       NOACPCT + SQLPCT + med_mkt_val"),
                   fit_08 = as.formula("~1 + HI_MAX + over65PCT + under5PCT +
                                       blackPCT + hispanicPCT + NOACPCT +
                                       SQLPCT + med_mkt_val"),
                   fit_09 = as.formula("~1 + (HI_MAX + over65PCT +
                                       under5PCT)^2"),
                   fit_10 = as.formula("~1 + (HI_MAX + blackPCT +
                                       hispanicPCT)^2"),
                   fit_11 = as.formula("~1 + (HI_MAX + NOACPCT + SQLPCT
                                       + med_mkt_val)^2"),
                   fit_12 = as.formula("~1 + (HI_MAX + over65PCT + under5PCT +
                                       blackPCT + hispanicPCT)^2"),
                   fit_13 = as.formula("~1 + (HI_MAX + over65PCT + under5PCT +
                                       NOACPCT + SQLPCT
                                       + med_mkt_val)^2"),
                   fit_14 = as.formula("~1 + (HI_MAX + blackPCT + hispanicPCT +
                                       NOACPCT + SQLPCT
                                       + med_mkt_val)^2"),
                   fit_15 = as.formula("~1 + (HI_MAX + over65PCT + under5PCT +
                                       BlackPCT + HispanicPCT + NOACPCT + SQLPCT
                                       + med_mkt_val)^2"))

n_draws <- 6000
thin_factor <- 10
amcmc.it <- n_draws / 2 * thin_factor
n_mods <- length(model_vars)

save_draws <- function(model_index) {
  draws <- MHGibbs(n_draws, thin_factor,
                   init.lstar.911,
                   init.lstar.death,
                   init.lstar.mu,
                   model_index)
  file_name <- paste(names(model_vars)[model_index], "_draws.RData", sep = "")
  save(draws, file = file_name)
}
